...
LLM providers (provider-agnostic Protocol; configured via env):
  ...
  Optional per-user AI Pass OAuth transport (server-owned tokens, live models)
...
5. Foreground LLM dispatch may use the platform provider, a selected BYOK
   credential, or an active AI Pass account. AI Pass bearer tokens remain
   encrypted server-side; worker jobs carry only an opaque connection ID.
