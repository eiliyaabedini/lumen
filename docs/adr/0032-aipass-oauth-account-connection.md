...
- Keep upstream source blank and configurable. A fork-owned private preview may
  inject an AI Pass-owned evaluation public client identifier through protected
  CI/build configuration only for a callback already registered to that client.
  This is evaluation-only, not an API key or client secret, and not upstream
  release guidance. It must never be committed, printed, logged, or returned.
